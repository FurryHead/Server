...
public class Bonuses {
	...
	public static int ATKSTAB = 0, ATKSLASH = 1, ATKCRUSH = 2, MAGIC_ATTACK = 3,
			ATKRANGE = 4, DEFSTAB = 5, DEFSLASH = 6, DEFCRUSH = 7,
			MAGIC_DEFENCE = 8, DEFRANGE = 9, STRENGTH = 10, PRAYER = 11;
	...
	public void sendBonus() {
		...
		for(int i = 0; i < (bonuses.length-1); i++) {
			player.getActionSender().sendString((Bonuses.BONUS_NAMES[i] + ": " + (bonuses[i] > 0 ? "+" : "") + bonuses[i]), 465, id++);
			if(i == 9) {
				id++;
				player.getActionSender().sendString((Bonuses.BONUS_NAMES[11] + ": " + (bonuses[11] > 0 ? "+" : "") + bonuses[11]), 465, id++);
			}
			if(i == 10) {
				id++;
				player.getActionSender().sendString((Bonuses.BONUS_NAMES[11] + ": " + (bonuses[11] > 0 ? "+" : "") + bonuses[11]), 465, id++);
