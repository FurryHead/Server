...
import org.hyperion.rs2.model.content.combat.CombatAssistant;
import org.hyperion.rs2.model.content.combat.CombatType;
import org.hyperion.rs2.model.content.combat.HitQueue;
...
import org.hyperion.rs2.tickable.impl.DeathTickable;
...
public class Player extends Entity implements Persistable {
	...
	/**
	 * Map of bonuses for the equipment the player is wearing.
	 */
	private Map<Integer, Integer> bonusList = new HashMap<Integer, Integer>();
	...
	/**
	 * Gets the hit queue of this player.
	 * @return The hit queue of this player.
	 */
	...
	/**
	 * The hit queue of this player.
	 */
	private HitQueue hitQueue = new HitQueue(this);
	
	/**
	 * Gets the hitpoints for this player.
	 * @return The hitpoints for this player.
	 */
	...
	/**
	 * The combat assistant for this player.
	 */
	private CombatAssistant combatAssistant = new CombatAssistant(this);
	
	/**
	 * Gets the combat assistant for this player.
	 * @return The combat assistant for this player.
	 */
	...
	/**
	 * Gets a certain bonus from the bonus list.
	 * @param id The bonus ID number.
	 * @return The bonus.
	 */
	public int getBonuses(int id) {
		if(bonusList.get(id) == null) {
			return 0;
		}
	...
	/**
	 * A map holding all the player's bonuses. 
	 * @return The player's bonus list.
	 */
	...
	public void inflictDamage(Hit inc, Entity source) {
		...
		if((source instanceof Entity) && (source != null)) {
			...
			if(this.isAutoRetaliating()) {
				...
				this.getActionQueue().addAction(new AttackAction(this, source, CombatType.MELEE));
			}
		}
			...
			if(!this.isDead()) {
				World.getWorld().submit(new Tickable(2) {
					...
					public void execute() {
						...
						getCombatAssistant().setAttackAction(null);
						playAnimation(Animation.create(836));
						this.stop();
					}
					
				});
				World.getWorld().submit(new DeathTickable(this));
			}
			this.setDead(true);
		}
	}
