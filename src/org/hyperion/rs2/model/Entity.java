...
public abstract class Entity {
	 ...
	 * @return The attribute value of the key, or <code>0</code> if the key was not found.
	 */
	...
	/**
	 * Gets the attribute map for this entity.
	 * @return The attribuet mapf or this entity.
	 */
	...
	/**
	 * Resets all of the walking attributes.
	 */
	public void resetWalkingAttributes() {
		String[] attr = { "WALKING_TO_ATTACK" };
		for(String att : attr) {
	...
	/**
	 * Checks whether or not this entity has a specific attribute key.
	 * @param attribute The attribute to check.
	 * @return <code>true</code> if this entity does has the attribute, <code>false</code> if not.
	 */
	...
	/**
	 * Gets this entity's list of attributes.
	 * @return This entity's list of attributes.
	 */
		...
		for(Region r : currentRegion.getSurroundingRegions()) {
			for(Player p : r.getPlayers()) {
				if(p.getLocation().isWithinDistance(this.getLocation())) {
					//p.getActionSender().sendLeetProjectile(projectile);
					p.getActionSender().sendProjectile(projectile.getStart(), projectile.getFinish(), projectile.getId(),
							projectile.getDelay(), projectile.getAngle(), projectile.getSpeed(), 
							projectile.getStartHeight(), projectile.getEndHeight(),
							projectile.getLockon().getLockonIndex(), projectile.getSlope(), projectile.getRadius());
				}
			}
		}
	}
	...
	/**
	 * Gets the entity's lockon index.
	 * @return The lockon index.
	 */
