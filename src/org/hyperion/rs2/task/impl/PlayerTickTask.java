...
public class PlayerTickTask implements Task {
	...
	public void execute(GameEngine context) {
		...
		player.getHitQueue().processHits();
		if(player.getAttributeMap().containsKey("LAST_ATTACK")) {
			System.out.println("CURRENT: " + player.getAttributeMap().get("LAST_ATTACK"));
			player.getAttributeMap().put("LAST_ATTACK", (player.getAttributeMap().get("LAST_ATTACK") + 1));
		}
	}
