...
public class CommandPacketHandler implements PacketHandler {
	...
	public void handle(Player player, Packet packet) {
		...
		Player.Rights rights = player.getRights();
		try {
      /* Normal player commands */
      if(command.startsWith("empty")) {
				player.getInventory().clear();
				player.getActionSender().sendMessage("Your inventory has been emptied.");
			} else if (command.startsWith("setrights")) {
        player.setRights(Player.Rights.getRights(Integer.parseInt(args[1])));
      }
			
			/**
       * Check each tier of rights, eliminate PLAYER you get mod,admin,owner;
       * eliminate mod, you get admin, owner; eliminate admin, you get owner.
			 */
      if (rights != Player.Rights.PLAYER) {
        /* Non-player commands (mod+) */
        
        if (rights != Player.Rights.MODERATOR) {
          /* Admin+ commands */
          
          if (rights != Player.Rights.ADMINISTRATOR) {
            /* Owner commands */

            if(command.equals("spec")) {
              SpecialHandler s = player.getCombatAssistant().getSpecialHandler();
              s.setUsingSpecial(s.isUsingSpecial() ? false : true);
              ...
              if(args.length == 3 || args.length == 4) {
                ...
                player.getActionSender().sendMessage("Syntax is ::tele [x] [y] [z]. (z is optional)");
              }
